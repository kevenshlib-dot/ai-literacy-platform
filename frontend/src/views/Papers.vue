<template>
  <div class="page-container">
    ...
    >
      ...
      <a-form layout="vertical">
        ...
        <a-form-item v-if="importModal.fileList.length > 0">
          ...
          >
            <template #icon><SearchOutlined /></template>
            {{ importModal.previewing ? 'AI 解析中...' : '智能解析预览' }}
          </a-button>
    ...
    <!-- Interactive Import Preview Drawer -->
    <a-drawer
      v-model:open="importPreviewDrawer"
      title="试卷智能解析预览"
      placement="right"
      :width="'90%'"
    ...
    >
      <PaperImportPreview
        v-if="importModal.preview?.parsed_data"
        :previewData="importModal.preview"
        @cancel="importPreviewDrawer = false"
...
import PaperImportPreview from '@/components/PaperImportPreview.vue'
...
const importPreviewDrawer = ref(false)
...
async function handleImportPreview() {
  ...
  await checkLLMModule('paper_import', '试卷智能解析')
...
function onPreviewImported(paper: any) {
  importPreviewDrawer.value = false
