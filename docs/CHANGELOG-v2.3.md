...
| 类型 | 内容 |
|------|------|
| 修复 | 所有模型 Enum 列添加 `values_callable`，彻底解决 PostgreSQL enum 大小写不匹配（判断题变简答题）|
| 修复 | DB 枚举值全部重命名为小写（`ALTER TYPE ... RENAME VALUE`）|
| 优化 | LLM 调用改为 async 线程池执行，不再阻塞 uvicorn 事件循环 |
| 优化 | `make_openai_client` 超时从 60s 提升到 300s（适配本地模型 thinking 模式）|
| 优化 | paper_parse_agent Prompt 加 `/no_think` 禁用 Qwen thinking，只发缺答案题目给 LLM |
| 新增 | 考试页面自动恢复进行中的考试（Exams.vue）+ "继续考试"横幅 |
| 新增 | 最后一题"下一题"按钮改为"交卷"按钮（Exams.vue）|
| 新增 | 试卷导入预览数据持久化到 sessionStorage，切页后可恢复 |

## v2.3.2 (2026-04-14)

| 类型 | 内容 |
|------|------|
| 修复 | 前端+后端双层智能题型检测：根据选项内容（T/F 键）自动纠正 `question_type`，彻底杜绝判断题显示文本框 |
| 修复 | 清理 DB 中因历史 bug 产生的脏数据（错标为 short_answer 的判断题）|
| 新增 | 种子数据更新：155 道题目、19 套试卷、12 场考试、样例 Word 文件 |
| 新增 | GitHub 协作者邀请：lp151709（刘鹏）、cxy630（陈晓扬）|
