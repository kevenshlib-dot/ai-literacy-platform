...
async def _prepare_material_generation_units(
    *,
    ...
    max_units: Optional[int],
    selection_mode: str,
    require_unique_capacity: bool = False,
) -> tuple[list[KnowledgeUnit], dict[str, int | str]]:
...
def _plan_unit_type_distribution(
    ...
    for question_type, total_count in positive_distribution:
        allocations = _allocate_counts_by_weights(weighted_items, total_count)
        for unit_id, count in allocations.items():
            if count > 0:
                plan[unit_id][question_type] = count
...
def _is_retryable_material_validation_reason(reason: str) -> bool:
    text = str(reason or "").strip()
    return (
        text == "未生成任何题目"
        or text.startswith("第")
        or text.startswith("同套")
    )
...
def _review_preview_questions(raw_questions: list[dict]) -> dict:
    """Compatibility hook for preview quality review.

    The current preview pipeline keeps AI review disabled by default for latency,
    but tests and callers still monkeypatch this hook to assert that the preview
    flow can bypass quality review deterministically.
    """
    return {
        "reviewed_count": 0,
        "warnings": [],
...
async def batch_delete(
    ...
    """Batch delete questions after cleaning up FK references."""
    if not question_ids:
        return 0
...
async def preview_question_bank_from_material(
    ...
    units, selection_stats = await _prepare_material_generation_units(
        ...
        max_units=max_units,
        selection_mode=selection_mode,
        require_unique_capacity=max_units is not None and max_units < requested_total,
    )
    ...
    for attempt in range(1, MATERIAL_GENERATION_MAX_ATTEMPTS + 1):
        ...
        for (qtype, slots), llm_result in zip(generation_batches, generation_results):
            ...
            if len(raw_questions) != len(slots):
                ...
                attempt_errors.append(mismatch_error)
                attempt_retry_errors.append(mismatch_error)
            for key in overall_usage:
        ...
        retryable_validation_reasons = [
            reason
            for reason in validation["reasons"]
            if _is_retryable_material_validation_reason(reason)
        ]
        should_retry = (
            bool(retryable_validation_reasons)
            or generated_total < requested_total
            or any("计划生成" in error and "实际返回" in error for error in attempt_errors)
        )
        quality_gate_failed = (
        ...
        if generated_total >= requested_total and not should_retry:
        ...
        if any("计划生成" in error and "实际返回" in error for error in attempt_errors):
            last_reasons = [
                error
                for error in attempt_errors
                if "计划生成" in error and "实际返回" in error
            ]
        elif retryable_validation_reasons:
            last_reasons = retryable_validation_reasons
        else:
        ...
        if retryable_validation_reasons:
            successful_questions.clear()
