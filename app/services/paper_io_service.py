...
async def import_paper(
    ...
    for sec_data in paper_data.get("sections", []):
        ...
        for q_item in sec_data.get("questions", []):
            ...
            # Extract correct_answer from either the override field or the question data
            q_data = q_item.get("question", {}) or {}
            correct_answer = (
                q_item.get("correct_answer_override")
            ...
            pq = PaperQuestion(
                ...
                question_type_override=q_item.get("question_type_override") or q_data.get("question_type"),
                correct_answer_override=correct_answer,
            )
            db.add(pq)
            total_score += score
            global_order += 1
