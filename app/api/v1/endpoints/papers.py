...
async def list_all_papers(
    ...
    creator_ids = list({p.created_by for p in papers if p.created_by})
    creator_map: dict = {}
    if creator_ids:
        from sqlalchemy import select as sa_select
        users_result = await db.execute(
            sa_select(UserModel.id, UserModel.username).where(UserModel.id.in_(creator_ids))
        )
    ...
    for p in papers:
        ...
        resp.creator_name = creator_map.get(p.created_by)
        data.append(resp)
...
async def preview_paper_file(
    ...
    try:
        ...
        for idx, pq in enumerate(all_questions, 1):
            ...
            if llm_enhanced:
                for llm_issue in (pq.get("issues") or []):
                    answer_issues.append({"question_index": idx, "message": f"[AI] {llm_issue}"})

            # Flag missing answers for objective types
            if not answer and qt in ("single_choice", "multiple_choice", "true_false"):
                msg = f"第{idx}题（{TYPE_LABELS.get(qt, qt)}）缺少参考答案"
                answer_issues.append({"question_index": idx, "message": msg})
...
@router.post("/import-reviewed", response_model=PaperResponse)
async def import_reviewed_paper(
    data: dict = Body(...),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_role(["admin", "organizer"])),
):
    """Import a paper from user-reviewed/corrected parsed data.

    Accepts the parsed_data dict (from preview-file response) with user corrections applied.
    Strips LLM-only fields and imports via the standard import_paper() service.
    """
    from app.services.paper_io_service import import_paper

    # Strip LLM enhancement fields from each question before import
    LLM_FIELDS = {"llm_question_type", "llm_correct_answer", "type_confidence",
                   "answer_confidence", "issues", "llm_type_differs"}
    ...
    for sec in paper.get("sections", []):
        for q in sec.get("questions", []):
            for f in LLM_FIELDS:
                q.pop(f, None)
    for q in paper.get("unsectioned_questions", []):
        for f in LLM_FIELDS:
            q.pop(f, None)

    # Remove top-level LLM flags
    data.pop("llm_enhanced", None)

    try:
        imported = await import_paper(db, data, current_user.id)
        await db.commit()
        await db.refresh(imported, attribute_names=["sections", "questions"])

        return PaperResponse.model_validate(imported)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
