...
def check_llm_status() -> dict:
    ...
    Returns:
        {
            "configured": bool,  # True if at least one module has LLM configured
            "modules": {module_key: {"name": str, "configured": bool, "provider": str}},
            "unconfigured_modules": [{"key": str, "name": str}],
        }
    """
    modules = {}
    unconfigured = []

    for key, name in ALL_LLM_MODULES:
        cfg = get_llm_config_sync(key)
        is_configured = cfg.api_key != "your-api-key" and cfg.base_url != ""
        provider_info = f"{cfg.model}@{cfg.base_url}" if is_configured else ""
        modules[key] = {
            "name": name,
        ...
        if not is_configured:
            unconfigured.append({"key": key, "name": name})

    return {
        "configured": any(m["configured"] for m in modules.values()),
        "modules": modules,
