...
def upgrade() -> None:
    ...
    if not _table_exists('papers'):
        op.create_table(
            ...
            sa.Column('description', sa.Text(), nullable=True),
            sa.Column('status', sa.Enum('draft', 'published', 'archived', name='paperstatus', create_type=False), nullable=False, server_default='draft'),
    ...
    if not _table_exists('interactive_sessions'):
        op.create_table(
            ...
            sa.Column('status', sa.Enum('active', 'completed', 'abandoned', name='interactivesessionstatus', create_type=False), nullable=False, server_default='active'),
    ...
    if not _table_exists('indicator_proposals'):
        op.create_table(
            ...
            sa.Column('status', sa.Enum('draft', 'submitted', 'under_review', 'approved', 'rejected', name='proposalstatus', create_type=False), nullable=False, server_default='draft'),
    ...
    if not _table_exists('courses'):
        op.create_table(
            ...
            sa.Column('status', sa.Enum('draft', 'published', 'archived', name='coursestatus', create_type=False), nullable=False, server_default='draft'),
            sa.Column('tags', JSONB, nullable=True),
            sa.Column('created_by', UUID(as_uuid=True), sa.ForeignKey('users.id'), nullable=False),
            sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.func.now()),
            sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.func.now()),
    ...
    if not _table_exists('learning_paths'):
        op.create_table(
            ...
            sa.Column('title', sa.String(200), nullable=False),
            sa.Column('description', sa.Text(), nullable=True),
            sa.Column('status', sa.Enum('active', 'completed', 'abandoned', name='learningpathstatus', create_type=False), nullable=False, server_default='active'),
    ...
    if not _table_exists('learning_steps'):
        op.create_table(
            ...
            sa.Column('order_num', sa.Integer(), nullable=False),
            sa.Column('step_type', sa.Enum('course', 'quiz', 'practice', 'reading', 'sandbox', name='learningsteptype', create_type=False), nullable=False),
            sa.Column('title', sa.String(200), nullable=False),
            sa.Column('description', sa.Text(), nullable=True),
            sa.Column('dimension', sa.String(100), nullable=True),
            sa.Column('resource_id', UUID(as_uuid=True), nullable=True),
            sa.Column('status', sa.Enum('pending', 'in_progress', 'completed', 'skipped', name='learningstepstatus', create_type=False), nullable=False, server_default='pending'),
    ...
    if not _table_exists('sandbox_sessions'):
        op.create_table(
            ...
            sa.Column('id', UUID(as_uuid=True), primary_key=True),
            sa.Column('user_id', UUID(as_uuid=True), sa.ForeignKey('users.id'), nullable=False, index=True),
            sa.Column('sandbox_type', sa.Enum('prompt_engineering', 'data_analysis', 'ethical_reasoning', 'tool_usage', 'creative_writing', name='sandboxtype', create_type=False), nullable=False),
            sa.Column('title', sa.String(200), nullable=False),
            sa.Column('description', sa.Text(), nullable=True),
            sa.Column('task_prompt', sa.Text(), nullable=False),
            sa.Column('dimension', sa.String(100), nullable=True, index=True),
            sa.Column('difficulty', sa.Integer(), nullable=False, server_default='3'),
            sa.Column('status', sa.Enum('active', 'completed', 'abandoned', name='sandboxsessionstatus', create_type=False), nullable=False, server_default='active'),
